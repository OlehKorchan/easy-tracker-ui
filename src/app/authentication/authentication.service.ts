...
import { IRegisterResponse } from './register/register-response';
import { ILoginRequest } from './login/login-request';
import { IRegisterRequest } from './register/register-request';
...
export class AuthenticationService {
  ...
  login(loginRequest: ILoginRequest): Observable<ILoginResponse> {
    return this._httpClient
      .post<ILoginResponse>(
        this._config.getApiUrl() + this._config.getLoginUrl(),
        loginRequest
      )
      .pipe(
  ...
  register(registerRequest: IRegisterRequest): Observable<IRegisterResponse> {
    return this._httpClient
      .post<IRegisterResponse>(
        this._config.getApiUrl() + this._config.getRegisterUrl(),
        registerRequest
      )
      .pipe(
        tap((data) => this.setUserSession(data)),
  ...
  logout() {
    return this._httpClient
      .post(this._config.getApiUrl() + this._config.getLogoutUrl(), null, {
        observe: 'response',
      })
      .pipe(
        tap((response) => {
  ...
  private setUserSession(userData: ILoginResponse | IRegisterResponse): void {
    ...
    localStorage.setItem(
      this._config.getUserNameSessionKey(),
      userData.username
    );
    localStorage.setItem(
      this._config.getExpiresAtSessionKey(),
