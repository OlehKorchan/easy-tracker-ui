...
import { ISpendingCategoryResponse } from 'src/app/models/spending-category-response';
...
export class CategoriesStatisticsComponent implements OnInit, OnDestroy {
  ...
  deleteCategory(id: string) {
    this._categoryService.removeCategory(id).subscribe({
      ...
      error: (error) =>
        console.error('Category deletion error: ', error?.error?.errorMessage),
    });
  }

  ngOnInit(): void {
    this.categoriesSubscription = this._categoryService
      .getUserCategories()
      .subscribe({
